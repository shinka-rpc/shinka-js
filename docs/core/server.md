...
event handlers can be registered once on the server and shared by all
connections.
...
server.onDataEvent("log", (data, bus) => console.log("Client:", data));
server.addEventListener("error", console.error);

server.start();
```

The exact transport implementation is independent of `Server`. See the
[transport documentation](../transports/) for details.
...
The `Bus` is the representation of the actual connection. Consequently, handlers
registered on the server receive the corresponding `Bus` as their `thisArg`:
...
This makes it possible to distinguish clients and communicate with a specific
connection.
...
See the [`Bus` documentation](./bus.md) for details about connection-level
operations.

## `Server` vs `Client`

`Client` and `Server` describe different **communication relationships**, not
necessarily different runtime roles.
...
The name of the transport does not determine whether an endpoint is a `Client`
or a `Server`.

For example, with a `DedicatedWorker`, both sides can be represented by `Client`,
because there is only one peer on each side. A `Server` is appropriate when one
endpoint manages multiple independent connections.
...
The transport is responsible for accepting incoming connections and notifying
`Server` when a new connection is available. `Server` then creates a `Bus` for
that connection.

The transport is deliberately independent from the `Server` abstraction and can
be implemented over different communication mechanisms.
...
Implementation of the exclusive-lock mechanism used by connections managed by
the server.
...
The handler receives the request key, request data, and the `Bus` representing
...
This means that you normally register a handler once rather than registering it
...
```ts
server.onDataEvent("message", (data, bus) => console.log("Received:", data));
```
...
`Server` exposes connection-level event listeners inherited from its internal
connection manager:
...
Once started, the server's transport may call the internal connection callback
whenever a new peer connects. Each connection is turned into a new `Bus`.
...
The server emits `predisconnect` before closing its connections and
`postdisconnect` after all connections have been disposed.

After `stop()` completes, the server returns to the stopped state and can be
started again.
...
The value represents the number of `Bus` instances currently managed by the
server.
...
They describe the lifecycle of the **server itself**, rather than individual
connections.
...
```ts
server.addEventListener("connect", () => console.log("Server started"));
```

It does **not** mean that a client has connected.

For individual connection events, use the server's `connect`/`disconnect` events
for `Bus` instances.
...
At this point the server has stopped accepting new connections and is about to
dispose of its active connections.
...
The library does not assign any semantics to this object, so applications can
use it to attach arbitrary state or metadata.
...
`Server` is built on top of [`Hub`](#), which manages the collection of
connection-level `Bus` instances.

Most applications should use `Server` directly. `Hub` is exposed primarily for
advanced use cases where the application needs to manage connection creation
independently from a specific server transport.
